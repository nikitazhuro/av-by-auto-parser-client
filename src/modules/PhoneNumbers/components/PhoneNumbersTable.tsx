...
const PhoneNumbersTable: React.FC<IPhoneNumbersTable> = ({
  isLoading,
  data,
}) => {
  ...
  const columns = [
      ...
      render: (_: any, row: any) => {
        return (
          <Image
            ...
            height={100}
            src={row.mileageCar.data?.avbyPhotosLinks?.[0]}
    ...
    },
    {
      ...
      render: (cell: any, row: any) => {
        const { brand, model, generation, year } = row.mileageCar?.data?.properties || {};
    ...
    {
      title: 'Price',
      dataIndex: ['mileageCar', 'data', 'price', 'usd', 'amount'],
      key: 'price',
      render: (cell: any) => {
        return <div>{cell || 'No data'}</div>
      }
    ...
    {
      title: 'AV link',
      dataIndex: ['mileageCar', 'data', 'publicUrl'],
      key: 'link',
      render: (cell: any) => (
  ...
  return (
    <Table
      ...
      className={classes.phoneNumbersTable}
      columns={columns}
      dataSource={data}
    />
  )
